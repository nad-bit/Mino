...
if [ -f "$APP_NAME.app/Contents/MacOS/$APP_NAME" ]; then
    ...
    if [ $? -eq 0 ]; then
        ...
        zip -qr "${APP_NAME}_v1.2.4_AppleSilicon.zip" "$APP_NAME.app"
        ...
        if [ $? -eq 0 ]; then
            ...
            zip -qr "${APP_NAME}_v1.2.4_Intel.zip" "$APP_NAME.app"
        ...
        zip -qr "${APP_NAME}_v1.2.4_Universal.zip" "$APP_NAME.app"
    fi
fi
...
SHA_ARM=$(shasum -a 256 "${APP_NAME}_v1.2.4_AppleSilicon.zip" | awk '{print $1}')
SHA_INTEL=$(shasum -a 256 "${APP_NAME}_v1.2.4_Intel.zip" | awk '{print $1}')
